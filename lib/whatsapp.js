...
class WhatsAppClient {
  constructor(config = {}) {
    this.config = Object.assign({
      ...
      sessionId: 'session1', // Default session ID, should be unique per instance
      chromePath: this._findChrome() // Auto-detect Chrome or use env variable
    }, config);
  ...
  /**
   * Auto-detect Chrome/Chromium installation or use environment variable
   * @returns {string|undefined} Path to Chrome executable or undefined to use Puppeteer's bundled Chromium
   */
  _findChrome() {
    // If user explicitly set CHROME_PATH, use it
    if (process.env.CHROME_PATH) {
      logger.log(`Using Chrome from CHROME_PATH: ${process.env.CHROME_PATH}`);
    ...
    try {
      ...
      if (whichChrome && fs.existsSync(whichChrome)) {
        logger.log(`✅ Auto-detected Chrome: ${whichChrome}`);
        return whichChrome;
      }
    } catch (e) {
      // 'which' command failed, continue to manual checks
    }

    // Check common paths
    for (const chromePath of chromePaths) {
      if (fs.existsSync(chromePath)) {
        logger.log(`✅ Auto-detected Chrome: ${chromePath}`);
        return chromePath;
      }
    }

    // If no Chrome found, log helpful installation instructions
    logger.log('⚠️  Chrome/Chromium not found on system');
    logger.log('📋 For Digital Ocean VPS or Ubuntu/Debian, install Chrome with:');
    logger.log('   wget -q -O - https://dl-ssl.google.com/linux/linux_signing_key.pub | sudo apt-key add -');
    logger.log('   sudo sh -c \'echo "deb [arch=amd64] http://dl.google.com/linux/chrome/deb/ stable main" >> /etc/apt/sources.list.d/google-chrome.list\'');
    logger.log('   sudo apt-get update');
    logger.log('   sudo apt-get install -y google-chrome-stable');
    logger.log('💡 Or install required dependencies for Puppeteer bundled Chromium:');
    logger.log('   sudo apt-get install -y ca-certificates fonts-liberation libappindicator3-1 libasound2 libatk-bridge2.0-0 libatk1.0-0 libc6 libcairo2 libcups2 libdbus-1-3 libexpat1 libfontconfig1 libgbm1 libgcc1 libglib2.0-0 libgtk-3-0 libnspr4 libnss3 libpango-1.0-0 libpangocairo-1.0-0 libstdc++6 libx11-6 libx11-xcb1 libxcb1 libxcomposite1 libxcursor1 libxdamage1 libxext6 libxfixes3 libxi6 libxrandr2 libxrender1 libxss1 libxtst6 lsb-release wget xdg-utils');
    logger.log('🔄 Using Puppeteer bundled Chromium (will attempt to download on initialization)');
  ...
  async initialize() {
    ...
    try {
      ...
      // Only set executablePath if Chrome was found
      if (this.config.chromePath) {
        puppeteerConfig.executablePath = this.config.chromePath;
      }
   ...
   * Note: Large files (>50MB) should be handled carefully to avoid memory issues
   */
  async sendMedia(phoneNumber, buffer, mimetype, caption, fileName) {
    ...
    if (fileSizeMB > 50) {
      logger.log(`⚠️ Warning: Sending large file (${fileSizeMB.toFixed(2)}MB). This may cause memory issues.`);
    }
